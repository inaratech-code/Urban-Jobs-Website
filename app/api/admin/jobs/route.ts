...
export async function GET(request: Request) {
  try {
    ...
    const jobs = await withTtlCache(`admin:jobs:${limit}`, 5000, async () => {
      const db = getAdminDb();
      const snap = await db.collection("jobs").orderBy("createdAt", "desc").limit(limit).get();
      return snap.docs.map((d) => {
        const data = d.data();
        // Normalize Timestamp to plain seconds/nanos so existing UI typing continues to work.
...
export async function PATCH(request: Request) {
  try {
    ...
    const db = getAdminDb();
    await db.collection("jobs").doc(body.id).update(body.data);
...
export async function DELETE(request: Request) {
  try {
    ...
    const db = getAdminDb();
    await db.collection("jobs").doc(body.id).delete();
    return NextResponse.json({ ok: true });
  } catch (e) {
    const msg = e instanceof Error ? e.message : "Unauthorized.";
    return NextResponse.json({ error: msg }, { status: 401 });
